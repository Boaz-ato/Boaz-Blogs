---
title: "Stabiliser Formalism"
weight: 9
math: true
---

So far, we have looked at the 3-qubit bit-flip code, 3-qubit phase code, and Shor's code. In all of these codes, we measured the eigenvalue of some Pauli operators and used the syndrome to determine the type of error that occurred. Here, we will formalise this concept using the stabiliser formalism.

## The Pauli Group
Below are the Pauli matrices for single qubits:
$$
I =
\begin{pmatrix}
1 & 0 \\
0 & 1
\end{pmatrix},
\qquad
X =
\begin{pmatrix}
0 & 1 \\
1 & 0
\end{pmatrix},
\qquad
Y =
\begin{pmatrix}
0 & -i \\
i & 0
\end{pmatrix},
\qquad
Z =
\begin{pmatrix}
1 & 0 \\
0 & -1
\end{pmatrix}
$$

#### Properties of Pauli Matrices

All four Pauli matrices are both unitary and Hermitian.

The non-identity Pauli matrices anticommute with one another:

$$XY = -YX  \qquad       XZ = -ZX  \qquad      YZ = -ZY$$


The Pauli matrices follow the multiplication rules:

$$ XX = YY = ZZ = I \qquad XY = iZ \qquad YZ = iX \qquad ZX = iY $$

That is, each Pauli matrix is its own inverse, and multiplying two different non-identity Pauli matrices together is always $\pm i$ times the remaining non-identity Pauli matrix.

#### Pauli Operations on Multiple Qubits

We can tensor the Pauli matrices to obtain operators that act on multiple qubits. For instance, the tensor product of the Pauli matrices below acts on four qubits:
$$ X \otimes Y \otimes I \otimes I$$
$$ Z \otimes Y \otimes I \otimes Z$$
$$ I \otimes X \otimes I \otimes I$$

We define the **weight** of $M \in P_n$ as the number of qubits on which $M$ acts as a non-identity operator. In our example above, the weights of the tensor product of the Pauli matrices are 2, 3 and 1. 


## The Pauli Observables

Pauli matrices, and $n$-qubit Pauli operations, are Hermitian matrices and as such can be viewed as measurements.

For an arbitrary Hermitian matrix $A$, the possible outcomes of a measurement are the distinct eigenvalues of $A$.

Using the spectral theorem, we can write an observable $A$ as a linear sum over its eigenvectors:
$$
A = \sum_{k = 1}^m \lambda_k \lvert\psi_k\rangle\langle\psi_k\rvert
$$

where $\lambda_k$ is the eigenvalue corresponding to the eigenstate $\lvert\psi_k\rangle$.

For Pauli operations, there are only two possibilities for the eigenvalues: $+1$ and $-1$. As an example, let's take the single-qubit Pauli-$X$ matrix. The spectral decomposition is given as follows:

$$
X = \lvert + \rangle \langle + \rvert - \lvert - \rangle \langle - \rvert
$$


From the decomposition, we can see that the possible eigenvalues are $+1$ and $-1$. As such, measuring the Pauli-$X$ observable on a single-qubit state would yield either the $\lvert + \rangle$ state or the $\lvert - \rangle$ state.

Let's also consider the 2-qubit Pauli operation $Z \otimes Z$. The spectral decomposition of this observable is:

$$
Z \otimes Z= \left( \lvert 00 \rangle \langle 00 \rvert + \lvert 11 \rangle \langle 11 \rvert \right) -
\left( \lvert 01 \rangle \langle 01 \rvert + \lvert 10 \rangle \langle 10 \rvert \right)
$$

In this case, we see that the eigenstates $\lvert 00 \rangle$ and $\lvert 11 \rangle$ have eigenvalue $+1$, while $\lvert 01 \rangle$ and $\lvert 10 \rangle$ have eigenvalue $-1$. As such, measurement projects the state onto either the subspace spanned by $\{\lvert 00 \rangle, \lvert 11 \rangle\}$ or by $\{\lvert 01 \rangle, \lvert 10 \rangle\}$.

In general, measurement of Pauli operations on $n$ qubits projects the state onto a subspace of the entire Hilbert space. More precisely, for every non-identity $n$-qubit Pauli operation, the $2^n$-dimensional state space always splits into two equal-dimensional subspaces of eigenvectors.
The circuit below gives an illustration of how measurement of the Pauli observable is performed.

![Projective Measurement](/images/projective_general.png)


Note that the quantum state after measurment on the ancilla collaspses into the $\pm 1$ eigenstate of the Pauli oberservable. 

After the controlled-$P_i$ gate, the system--ancilla state is

$$
\frac{1}{\sqrt{2}}\big(\ket{\psi}\ket{0} + P_i\ket{\psi}\ket{1}\big).
$$

Applying a Hadamard to the ancilla
($\ket{0}\to\tfrac{1}{\sqrt2}(\ket{0}+\ket{1})$,
$\ket{1}\to\tfrac{1}{\sqrt2}(\ket{0}-\ket{1})$) gives

$$
\begin{aligned}
&\tfrac{1}{2}\ket{\psi}\ket{0} + \tfrac{1}{2}\ket{\psi}\ket{1} + \tfrac{1}{2}P_i\ket{\psi}\ket{0} - \tfrac{1}{2}P_i\ket{\psi}\ket{1} \\
&= \tfrac{1}{2}\big(\ket{\psi}+P_i\ket{\psi}\big)\ket{0} + \tfrac{1}{2}\big(\ket{\psi}-P_i\ket{\psi}\big)\ket{1} \\
&= \underbrace{\tfrac{1}{2}(I+P_i)}_{\Pi_+}\ket{\psi}\ket{0} + \underbrace{\tfrac{1}{2}(I-P_i)}_{\Pi_-}\ket{\psi}\ket{1}.
\end{aligned}
$$

Measuring the ancilla in the computational basis therefore projects the
system onto $\Pi_+\ket{\psi}$ (outcome $\ket{0}$) or $\Pi_-\ket{\psi}$
(outcome $\ket{1}$), where the two operators are

$$
\Pi_+ = \tfrac{1}{2}(I+P_i), \qquad \Pi_- = \tfrac{1}{2}(I-P_i).
$$

Using $P_i^2 = I$:

$$
\begin{aligned}
P_i\,\Pi_+ &= P_i\,\tfrac{1}{2}(I+P_i)
 = \tfrac{1}{2}\big(P_i+P_i^{2}\big)
 = \tfrac{1}{2}(P_i+I) = \Pi_+, \\
P_i\,\Pi_- &= P_i\,\tfrac{1}{2}(I-P_i)
 = \tfrac{1}{2}\big(P_i-P_i^{2}\big)
 = \tfrac{1}{2}(P_i-I) = -\tfrac{1}{2}(I-P_i) = -\Pi_-.
\end{aligned}
$$

Hence the two possible post-measurement states are eigenstates of $P_i$:

$$
\begin{aligned}
P_i\big(\Pi_+\ket{\psi}\big) &= +\,\Pi_+\ket{\psi}
 && \text{(eigenvalue } +1\text{)}, \\
P_i\big(\Pi_-\ket{\psi}\big) &= -\,\Pi_-\ket{\psi}
 && \text{(eigenvalue } -1\text{)}.
\end{aligned}
$$

So the circuit projects an arbitrary $\ket{\psi}$ into the $+1$ or $-1$
eigenspace of the Pauli observable $P_i$, with the ancilla outcome
determining which one.


## The Stabiliser Group

The Pauli operators can be used to generate a group called the **stabiliser** of a quantum code. An operator $M$ stabilises a state $|\psi\rangle$ if:

$$M|\psi\rangle = |\psi\rangle \quad \text{for all encoded states } |\psi\rangle.$$

The elements of the stabiliser group must be **minimal**: it must not be possible to obtain any element $G_i$ as a product of the other elements. For example, consider the set $S = \{Z_1Z_2,\, Z_2Z_3,\, Z_1Z_3\}$. This is not minimal, since $Z_1Z_3 = (Z_1Z_2)(Z_2Z_3)$. A minimal generating set is $S = \{Z_1Z_2,\, Z_2Z_3\}$.

Note also that the stabiliser set is closed under multiplication: if $M|\psi\rangle = |\psi\rangle$ and $N|\psi\rangle = |\psi\rangle$, then:

$$MN|\psi\rangle = |\psi\rangle.$$

So products of stabilisers are also stabilisers.

### The Abelian Condition

The stabiliser group must be **Abelian**. That is, every pair of elements must commute. This is necessary so that all stabilisers can be measured simultaneously, yielding a well-defined syndrome without the measurement order affecting the outcome.

**Proof.** Suppose $M, N \in S$, so $M|\psi\rangle = |\psi\rangle$ and $N|\psi\rangle = |\psi\rangle$. Consider the commutator acting on a codeword:

$$(MN - NM)|\psi\rangle = MN|\psi\rangle - NM|\psi\rangle = M|\psi\rangle - N|\psi\rangle = |\psi\rangle - |\psi\rangle = 0.$$

Since $M$ and $N$ are Pauli operators, they must either commute ($MN = NM$) or anticommute ($MN = -NM$). If they anticommuted, we would have:

$$(MN - NM)|\psi\rangle = MN|\psi\rangle - NM|\psi\rangle = |\psi\rangle -(-|\psi\rangle) = 2|\psi\rangle \neq 0,$$

which is a contradiction. Therefore $MN = NM$, and all elements of the stabiliser group must commute. $\square$

## The Codespace

Given an Abelian subgroup $S$ of $P_n$, we define the **codespace**:

$$T(S) = \left\{ |\psi\rangle \;\middle|\; M|\psi\rangle = |\psi\rangle \;\; \forall M \in S \right\}.$$

The codespace is a subspace of the full $2^n$-dimensional Hilbert space. Suppose we wish to encode $k$ logical qubits into $n$ physical qubits; then the stabiliser group $S$ has $n - k$ independent generators $G_1, \ldots, G_{n-k}$. Each generator has eigenvalues $\pm 1$, and measuring it splits the Hilbert space into two equal halves: the $+1$ eigenspace and the $-1$ eigenspace. Each successive generator measurement halves the remaining subspace, so after all $n - k$ measurements the joint $+1$ eigenspace has dimension $2^n / 2^{n-k} = 2^k$. This $2^k$-dimensional subspace is the codespace $T(S)$, encoding exactly $k$ logical qubits.

The syndrome — the list of $\pm 1$ eigenvalues obtained from measuring each generator — identifies which subspace the state lies in. An error is detected whenever any generator returns $-1$, signalling that the state has been pushed out of the codespace.

## Error Detection

Suppose $M \in S$ and a Pauli error $E$ **anticommutes** with $M$. Then:

$$M(E|\psi\rangle) = -EM|\psi\rangle = -E|\psi\rangle,$$

so $E|\psi\rangle$ has eigenvalue $-1$ for $M$. Conversely, if $E$ **commutes** with every $M \in S$:

$$M(E|\psi\rangle) = EM|\psi\rangle = E|\psi\rangle,$$

so $E|\psi\rangle$ has eigenvalue $+1$ for all stabilisers. The eigenvalue of $M$ therefore detects errors which anticommute with $M$.

## The Normaliser and Code Distance

Define the **normaliser** of $S$:

$$N(S) = \left\{ N \in P_n \;\middle|\; MN = NM \;\; \forall M \in S \right\}.$$

The **distance** $d$ of the code $T(S)$ is the minimum weight of any Pauli operator in $N(S) \setminus S$.

- Errors **not in** $N(S)$ are detected, as they anticommute with at least one stabiliser.
- Errors **in** $S$ leave all codewords unchanged and are called **degenerate errors** — they require no correction.
- Errors **in** $N(S) \setminus S$ commute with all stabilisers but act non-trivially on the codespace; these are the **logical operators** and cannot be detected.

To correct $t$ errors, the code must have distance $d \geq 2t + 1$.

Two errors $E$ and $F$ produce the same error syndrome if and only if $E^\dagger F \in N(S)$. If $E^\dagger F \notin N(S)$, the syndrome distinguishes them. If $E^\dagger F \in S$, the errors act identically on all codewords and there is no need to distinguish them.


## Summary

The stabilisers $\{G_i\}$ of an $[\![n, k, d]\!]$ code must satisfy the following properties:

1. **Pauli group elements:** Each $G_i \in P_n$.
2. **Stabilise all logical states:** $G_i |\psi\rangle_L = |\psi\rangle_L$ for all logical codewords $|\psi\rangle_L$.
3. **Mutual commutativity:** $[G_i, G_j] = 0$ for all $i, j$, so stabilisers can be measured simultaneously.

The stabiliser group $S = \langle G_1, \ldots, G_{n-k} \rangle$ is an Abelian subgroup of $P_n$. The code corrects any error set $\mathcal{E}$ such that $E^\dagger F \notin N(S) \setminus S$ for all pairs $E, F \in \mathcal{E}$.
